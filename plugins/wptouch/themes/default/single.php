...
<div class="content" id="content<?php echo md5($_SERVER['REQUEST_URI']); ?>">
		<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			<div class="post">
			  ...
			  <div class="single-post-meta-top">
			    <?php echo get_the_time('j M Y @ h:i a') ?> &rsaquo; <?php the_author() ?>
			    <br />
		      ...
		      <?php if ('open' == $post->comment_status) : ?>
			      <?php if (function_exists('dsq_comments_template')) { ?>
		       		<a href="#dsq-add-new-comment">&darr; <?php _e( "Vezi comentariile", "wptouch" ); ?></a>
			      <?php } elseif (function_exists('id_comments_template')) { ?>
				      <a href="#idc-container-parent">&darr; <?php _e( "Vezi comentariile", "wptouch" ); ?></a>
			      <?php } elseif (isset($post->comment_count) && $post->comment_count == 0) { ?>
				      <a href="#respond">&darr; <?php _e( "Fii primul care comenteaza", "wptouch" ); ?></a>
			      <?php } elseif (isset($post->comment_count) && $post->comment_count > 0) { ?>
				      <a href="#com-head">&darr; <?php _e( "Vezi comentariile", "wptouch" ); ?></a>
			      <?php } ?>
		      <?php endif; ?>
		      </div>
	    </div>
      ...
      <div class="post" id="post-<?php the_ID(); ?>">
        ...
        <!-- Categories and Tags post footer -->        
			  <div class="single-post-meta-bottom">
				  <?php link_pages('<div class="post-page-nav">' . __( "Articole similare", "wptouch" ) . ': ', '</div>', 'number', ' &raquo;', '&laquo; '); ?>          
			    <?php _e( "Categorii", "wptouch" ); ?>: <?php if (the_category(', ')) the_category(); ?>
			    <?php if (function_exists('get_the_tags')) the_tags('<br />' . __( 'Etichete', 'wptouch' ) . ': ', ', ', ''); ?>  
		    </div>   
    ...
    bloginfo('name'); ?>- <?php the_title();?>&body=<?php _e( "Check out this post:", "wptouch" ); ?>%20<?php the_permalink() ?>" onclick="return confirm('<?php _e( "Mail a link to this post?", "wptouch" ); ?>');" id="omail"></a></li>
		    ...
		    <li><a href="http://www.facebook.com/smuffgadget"><img src="<?php bloginfo('stylesheet_directory'); ?>/img/Facebook.png" /></a></li>
